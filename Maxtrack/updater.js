// ============================================================
// AUTO-UPDATER - Maxtrack Extension
...
const GITHUB_UPDATE_CONFIG = {
  ...
  folder: 'Maxtrack',                      // Pasta da extensão
...
class ExtensionUpdater {
  ...
  async checkForUpdates(manual = false) {
    ...
    try {
      ...
      if (this.isNewerVersion(versionData.version, this.currentVersion)) {
        ...
        if (manual) {
          this.showNotification('Maxtrack já está atualizado!', 'Você tem a versão mais recente.');
        }
      }
  ...
  async applyUpdate() {
    try {
      ...
      if (files['styles.css']) {
        ...
        const oldStyles = document.querySelectorAll('style[data-maxtrack-extension]');
        oldStyles.forEach(style => style.remove());

        // Injeta novos estilos
        const styleElement = document.createElement('style');
        styleElement.setAttribute('data-maxtrack-extension', 'true');
      ...
      for (const jsFile of jsFiles) {
        if (files[jsFile]) {
          ...
          try {
            ...
            const oldScript = document.querySelector(`script[data-maxtrack-file="${jsFile}"]`);
            if (oldScript) {
              oldScript.remove();
            }
            ...
            scriptElement.setAttribute('data-maxtrack-file', jsFile);
      ...
      if (needsReload) {
        ...
        this.showNotification(
          `Maxtrack v${version} - Quase pronto!`,
  ...
  showNotification(title, message) {
    ...
    const oldPopup = document.getElementById('maxtrack-update-popup');
    ...
    popup.id = 'maxtrack-update-popup';
    ...
    const popupContent = isSuccess ? `
      ...
      ">
        ...
        <div style="display: flex; align-items: center; margin-bottom: 16px;">
          ...
          <button id="maxtrack-popup-close" style="
        ...
        <!-- Botão OK -->
        <button id="maxtrack-popup-ok" style="
          background: white;
          color: #059669;
          border: none;
          padding: 14px 20px;
      ...
      ">
        ...
        ">
          ...
          <ol style="margin: 0; padding-left: 20px; font-size: 13px; line-height: 1.8;">
            ...
            <li>Clique em <strong>🔄 Recarregar</strong> na extensão Maxtrack</li>
          </ol>
        </div>

        <!-- Botões -->
        <button id="maxtrack-popup-copy" style="
          background: white;
          color: #667eea;
          border: none;
          padding: 14px 20px;
        ...
        <button id="maxtrack-popup-later" style="
    ...
    style.textContent = `
      ...
      #maxtrack-popup-close:hover {
        background: rgba(255,255,255,0.3) !important;
        transform: scale(1.1) rotate(90deg);
      }
      #maxtrack-popup-copy:hover {
        background: #f8f9ff !important;
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(0,0,0,0.2);
      }
      #maxtrack-popup-ok:hover {
        background: #f0fdf4 !important;
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(0,0,0,0.2);
      }
      #maxtrack-popup-later:hover {
    ...
    // Botão fechar
    const closeBtn = document.getElementById('maxtrack-popup-close');
    if (closeBtn) {
    ...
    const okBtn = document.getElementById('maxtrack-popup-ok');
    if (okBtn) {
    ...
    const copyBtn = document.getElementById('maxtrack-popup-copy');
    if (copyBtn) {
    ...
    const laterBtn = document.getElementById('maxtrack-popup-later');
    if (laterBtn) {
    ...
    setTimeout(() => {
      if (document.getElementById('maxtrack-update-popup')) {
        popup.style.animation = 'slideIn 0.3s ease-out reverse';
        setTimeout(() => popup.remove(), 300);
      }
    }, 30000);
