...
class ExtensionUpdater {
  ...
  async applyUpdate() {
    try {
      ...
      if (files['styles.css']) {
        ...
        const oldStyles = document.querySelectorAll('style[data-sighra-extension]');
        oldStyles.forEach(style => style.remove());

        // Injeta novos estilos
        const styleElement = document.createElement('style');
        styleElement.setAttribute('data-sighra-extension', 'true');
      ...
      for (const jsFile of jsFiles) {
        if (files[jsFile]) {
          ...
          try {
            ...
            const oldScript = document.querySelector(`script[data-sighra-file="${jsFile}"]`);
            if (oldScript) {
              oldScript.remove();
            }
            ...
            scriptElement.setAttribute('data-sighra-file', jsFile);
  ...
  showNotification(title, message) {
    ...
    const oldPopup = document.getElementById('sighra-update-popup');
    ...
    const popupContent = isSuccess ? `
      ...
      ">
        ...
        <div style="display: flex; align-items: center; margin-bottom: 16px;">
          ...
          <button id="sighra-popup-close" style="
        ...
        <!-- Botão OK -->
        <button id="sighra-popup-ok" style="
          background: white;
          color: #059669;
          border: none;
          padding: 14px 20px;
      ...
      ">
        ...
        ">
          ...
          <ol style="margin: 0; padding-left: 20px; font-size: 13px; line-height: 1.8;">
            ...
            <li>Clique em <strong>🔄 Recarregar</strong> na extensão Sighra</li>
          </ol>
        </div>

        <!-- Botões -->
        <button id="sighra-popup-copy" style="
          background: white;
          color: #667eea;
          border: none;
          padding: 14px 20px;
        ...
        <button id="sighra-popup-later" style="
    ...
    style.textContent = `
      ...
      #sighra-popup-close:hover {
        background: rgba(255,255,255,0.3) !important;
        transform: scale(1.1) rotate(90deg);
      }
      #sighra-popup-copy:hover {
        background: #f8f9ff !important;
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(0,0,0,0.2);
      }
      #sighra-popup-ok:hover {
        background: #f0fdf4 !important;
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(0,0,0,0.2);
      }
      #sighra-popup-later:hover {
    ...
    // Botão fechar
    const closeBtn = document.getElementById('sighra-popup-close');
    if (closeBtn) {
    ...
    const okBtn = document.getElementById('sighra-popup-ok');
    if (okBtn) {
    ...
    const copyBtn = document.getElementById('sighra-popup-copy');
    if (copyBtn) {
    ...
    const laterBtn = document.getElementById('sighra-popup-later');
    if (laterBtn) {
    ...
    setTimeout(() => {
      if (document.getElementById('sighra-update-popup')) {
        popup.style.animation = 'slideIn 0.3s ease-out reverse';
        setTimeout(() => popup.remove(), 300);
      }
    }, 30000);
